...
def stockx_main(product_sku, userrrrrsize=None):
    ...
    try:
        ...
        newapilink = 'https://stockx.com/api/products/' + \
            str(jsonshit['hits'][0]['url'])
        print(newapilink)
    except BaseException:
        skuuuuuuname = ''
        shoeeeename = ''
    ...
    if newapilink:
        try:
            productpagelink = newsession.get(
                newapilink +
                '?includes=market,360&children=&currency=USD',
                proxies=LoadLumProx(),
                headers=headers,
                timeout=7)
        ...
        try:
            ...
            last_sale = 'n/a'
        lastsoldsize = sizzeeess = '*Size :: ' + \
            str(last_size) + '* | $' + str(last_sale)
        try:
        ...
        try:
            ...
            highest_bid = 'n/a'
        highest_offer = sizzeeess = '*Size :: ' + \
            str(highest_size) + '* | $' + str(highest_bid)
            ...
            for key, value in json_dict['Product']['children'].items():
                ...
                if userrrrrsize:  # if shoe_size == 8:
                    if str(shoe_size) == str(userrrrrsize):
                        # if str(shoe_size) == '8':
                        shoe_size = shoe_size.replace(
                            '.5', '½')  # print(value['market'])
                        try:
                    ...
                    #    print(value[0]['market'])
                        ...
                        try:
                            shoe_saless = str(
                                shoe_deadstockSold) + ' | ' + str(shoe_lastSale)
                            shoe_saless = (
                                '{:11s}::{:5s} | {:5s}'.format(
                                    str(sizzeeess),
                                    str(shoe_deadstockSold),
                                    str(shoe_lastSale)))
                        except BaseException:
                            shoe_saless = ''
                        try:
                    ...
                    try:
                        shoe_saless = str(shoe_deadstockSold) + \
                            ' | ' + str(shoe_lastSale)
                        shoe_saless = (
                            '{:11s}::{:5s} | {:5s}'.format(
                                str(sizzeeess),
        ...
        try:
            sizes_of = 15
            splitthesesizes = [sizelist[i:i + sizes_of]
                               for i in range(0, len(sizelist), sizes_of)]
        except BaseException:
            splitthesesizes = []
        try:
        ...
        try:
            sizes_of = 15
            splitthesstocks = [stocklist[i:i + sizes_of]
                               for i in range(0, len(stocklist), sizes_of)]
        except BaseException:
            splitthesstocks = []
        try:
        ...
        try:
            ...
            if int(last_sale) > int(pricename) and int(
                    last_sale) < (int(pricename) + 50):
                slackcolor = '#ffff00'
                #shoename = ' :question_:  ' + str(shoename) + ' ' + ' :question_:  '
                shoe_worthhhh = ':question_:  ' + ' :question_:  '
            ...
            slackcolor = '#ffff00'
            shoename = ' :question_:  ' + \
                str(shoename) + ' ' + ' :question_:  '
        try:
            buylink = weblink.replace(
                'https://stockx.com/',
                'https://stockx.com/buy/')
        except BaseException:
            buylink = str(weblink)
        try:
            selllink = weblink.replace(
                'https://stockx.com/',
                'https://stockx.com/sell/')
        except BaseException:
            selllink = str(weblink)
        # newlist.append(totalstockinfo)
        # twitterstock = ('#sizes: ' + str(stockstatus))
        justthesizelist = ('::'.join(justthesizelist))
        ...
        checkingforchanges = (
            '{:15s}|| {:10s} ||{:10s}|| {:50s}||{:30s}||{:30s}||{}'.format(
                str(releasedate),
                str(skuname),
                str(total_sold),
                str(shoename),
        ...
        slackcheckcartlinks = str(slackcartlink) + \
            '   ||   ' + str(slackcheckoutlink)
        goatlink = "<" + 'https://www.goat.com/search?query=' + \
            str(skuname) + "|" + 'GOAT' + ">"
        ebaylink = "<" + 'http://www.ebay.com/sch/i.html?_nkw=' + \
            str(skuname) + "|" + 'EBAY' + ">"
        stockxlink = "<" + 'https://stockx.com/search?s=' + \
            str(skuname) + "|" + 'STOCKX' + ">"
        fclink = "<" + 'https://www.flightclub.com/catalogsearch/result/?q=' + \
            str(skuname) + "|" + 'FC' + ">"
        printstyledata = (
            'Style# : :: *' +
            str(skuname) +
        ...
        try:
            ...
            requests.post(
                slack_hook, data=json.dumps(payload), headers={
